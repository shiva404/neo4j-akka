...
public class UserDaoImpl implements UserDao {
	...
	public User getUserByGoodreadsId(String goodreadsId) {
		return userRepository.findBySchemaPropertyValue("goodreadsId",
				goodreadsId);
	}
	...
	public List<User> search(String searchString) {
		return userRepository.searchUsers(String.format(SEARCH_STRING,
				searchString));
	}

	@Override
	public List<User> searchFriends(String userId, String searchString) {
		return userRepository.searchFriends(userId,
				String.format(SEARCH_STRING, searchString));
	}
	...
	@Override
	// @TriggersRemove(cacheName = "userFollowing", keyGenerator =
	// @KeyGenerator(name = "HashCodeCacheKeyGenerator", properties =
	// @Property(name = "includeMethod", value = "false")))
	public void createFollowingRelation(@PartialCacheKey User user,
			User follower) {

		long now = System.currentTimeMillis();
		UserRelation userRelation = new UserRelation(user, follower, now,
				UserRelationType.FOLLOWING.toString());
		neo4jTemplate.save(userRelation);

		try {
			Long currentTime = System.currentTimeMillis();
			Target targetforAuditEvent = createTargetToUser(follower);
			Event followedUSerEvent = new Event(
					AuditEventType.FOLLOWING.toString(), targetforAuditEvent,
					currentTime, true);
	...
	if (existingRelation.size() == 1) {
			...
			if (relation.getType().equals(UserRelationType.FRIEND_REQUEST_PENDING.toString())) {
				...
				try {
					...
					String targetNoitficationUrl = "users/"
							+ targetNotificationUserId;
					...
					Event beFriendUserEvent1 = new Event(
							AuditEventType.FRIEND.toString(), targetEventUser,
							System.currentTimeMillis(), true);
					Event beFriendUserEvent2 = new Event(
							AuditEventType.FRIEND.toString(),
							targetEventFriend, System.currentTimeMillis(), true);
					Target targetNotification = new Target(
							IdType.USER_ID.toString(), targetNotificationstring
									+ " accepted your friend request",
							targetNoitficationUrl);

					Notification beFriendNotification = new Notification(
							targetNotification, currentTime);
	...
	@Override
	// @Cacheable(cacheName = "userFollowing", keyGenerator = @KeyGenerator(name
	// = "HashCodeCacheKeyGenerator", properties = @Property(name =
	// "includeMethod", value = "false")))
	public List<User> getFollowing(String userId) {
		return userRepository.getFollowing(userId);
	}

	@Override
	// @Cacheable(cacheName = "userOwnedBooks", keyGenerator =
	// @KeyGenerator(name = "HashCodeCacheKeyGenerator", properties =
	// @Property(name = "includeMethod", value = "false")))
	public List<OwnedBook> getOwnedBooks(String userId) {
		Map<String, Object> params = new HashMap<>();
		params.put("userId", userId);
		Result<Map<String, Object>> mapResult = neo4jTemplate
				.query("match (users:User {id: {userId}})-[relation:OWNS]->(books:Book) return books, relation",
						params);
	...
	public List<OwnedBook> getAvailableBooks(String userId) {
		...
		params.put("userId", userId);
		Result<Map<String, Object>> mapResult = neo4jTemplate
				.query("match (users:User {id: {userId}})-[relation:OWNS {status: \"available\"}]->(books:Book) return books, relation",
						params);
		return getOwnedBooksFromResultMap(mapResult);
	}
	...
	public List<OwnedBook> getLentBooks(String userId) {
		...
		params.put("userId", userId);
		Result<Map<String, Object>> mapResult = neo4jTemplate
				.query("match (users:User {id: {userId}})-[relation:OWNS {status: \"lent\"}]->(books:Book) return books, relation",
						params);
		return getOwnedBooksFromResultMap(mapResult);
	}

	private List<OwnedBook> getOwnedBooksFromResultMap(
		...
		for (Map<String, Object> objectMap : mapResult) {
			...
			RestRelationship rawOwnsRelationship = (RestRelationship) objectMap
					.get("relation");

			Book book = neo4jTemplate.convert(bookNode, Book.class);
			OwnsRelationship ownsRelationship = neo4jTemplate.convert(
					rawOwnsRelationship, OwnsRelationship.class);
	...
	@Override
	// @Cacheable(cacheName = "userBorrowedBooks", keyGenerator =
	// @KeyGenerator(name = "HashCodeCacheKeyGenerator", properties =
	// @Property(name = "includeMethod", value = "false")))
	public List<BorrowedBook> getBorrowedBooks(String userId) {
		...
		params.put("userId", userId);
		Result<Map<String, Object>> mapResult = neo4jTemplate
				.query("match (users:User {id: {userId}})-[relation:BORROWED]->(books:Book) return books, relation",
						params);
	...
	public void addAddressToUser(Address address, User user) {
		...
		try {
			...
			Target target = new Target(IdType.USER_ID.toString(), targetEvent,
					null);
			Event addAddressToUserEvent = new Event(
					AuditEventType.ADDED_ADDRESS.toString(), target,
					currentTime, false);
	...
	@Override
	// @Cacheable(cacheName = "userWishBooks", keyGenerator = @KeyGenerator(name
	// = "HashCodeCacheKeyGenerator", properties = @Property(name =
	// "includeMethod", value = "false")))
	public List<WishListBook> getWishListBooks(String userId) {
		...
		params.put("userId", userId);
		Result<Map<String, Object>> mapResult = neo4jTemplate
				.query("match (users:User {id: {userId}})-[relation:WISH]->(books:Book) return books, relation",
						params);
	...
	public void synchWishListRec(String userId) {
		...
		if (user != null)
			goodreadsAsynchHandler.getFriendRecForUser(userId,
					user.getGoodreadsId(), user.getGoodreadsAccessToken(),
					user.getGoodreadsAccessTokenSecret());
	}
	...
	public List<UserRecommendation> getUserRecommendations(String userId) {
		...
		params.put("userId", userId);
		Result<Map<String, Object>> mapResult = neo4jTemplate
				.query("match (users:User {id: {userId}})-[relation:GR_REC]->(books:Book) return books, relation",
						params);
		return getWishUserRecFromResultMap(mapResult);
	}

	private List<WishListBook> getWishListBooksFromResultMap(
			Result<Map<String, Object>> mapResult) {
		List<WishListBook> wishListBooks = new ArrayList<>();
		for (Map<String, Object> objectMap : mapResult) {
			RestNode bookNode = (RestNode) objectMap.get("books");
			RestRelationship rawWishRelationship = (RestRelationship) objectMap
					.get("relation");

			Book book = neo4jTemplate.convert(bookNode, Book.class);
			WishListRelationship whishListRelationship = neo4jTemplate.convert(
					rawWishRelationship, WishListRelationship.class);
			wishListBooks.add(new WishListBook(book, whishListRelationship));
		}
		return wishListBooks;
	}

	private List<UserRecommendation> getWishUserRecFromResultMap(
			Result<Map<String, Object>> mapResult) {
		...
		for (Map<String, Object> objectMap : mapResult) {
			...
			Book book = neo4jTemplate.convert(bookNode, Book.class);
			GoodreadsFriendBookRecRelation goodreadsFriendBookRecRelation = neo4jTemplate
					.convert(rawWishRelationship,
							GoodreadsFriendBookRecRelation.class);
			userRecommendations.add(new UserRecommendation(book,
					goodreadsFriendBookRecRelation));
		}
		return userRecommendations;
	}

	private List<BorrowedBook> getBorrowedBooksFromResultMap(
			Result<Map<String, Object>> mapResult) {
		...
		for (Map<String, Object> objectMap : mapResult) {
			...
			Book book = neo4jTemplate.convert(bookNode, Book.class);
			BorrowRelation borrowRelationship = neo4jTemplate.convert(
					rawOwnsRelationship, BorrowRelation.class);
	...
	public void deleteFriendRequest(String userId, String friendUserId) {
		...
		// userRelationRepository.getUsersRelationship(userId, friendUserId);
		User user = getUser(userId);
